...
%% load all images and set background to nan
BScan = single(zeros(size(BScan2save,1),size(BScan2save,2),size(BScan2save,3)));
nan_thresh = 55;
for item = 1:size(BScan2save,3)
    BScan_curr = single(BScan2save(:,:,item));
    BScan_curr(BScan_curr <= nan_thresh) = 0; %nan;
