...
for frm = 1:100 %FRM_NUM-1
    ...
    % store coordinates
    pc_x((frm - 1) * HEIGHT * WIDTH + 1:frm * HEIGHT * WIDTH) = xglobal * 1e3; % mm
...
valid_ind = find(pc_x == 0 & pc_y == 0 & pc_z == 0, 1);
pc_x(valid_ind:end) = [];
pc_y(valid_ind:end) = [];
pc_z(valid_ind:end) = [];
pc_int(valid_ind:end) = [];

fprintf('generate pointcloud took %f sec \n', toc);
clear xlocal ylocal zlocal xglobal yglobal zglobal
clear intensity row col slice probe

%% generate volume data from pointcloud
tic;
hscale = 0.5; wscale = 0.5; cscale = 0.3;
VOLUME_HEIGHT = round(HEIGHT * hscale);
VOLUME_WIDTH = round(WIDTH * length(data_sizes) * wscale);
VOLUME_CHANNEL = round(min(data_sizes) * cscale);
volume_raw = zeros(VOLUME_HEIGHT, VOLUME_WIDTH, VOLUME_CHANNEL, 'uint8');

res_ele = VOLUME_CHANNEL / (max(pc_x) - min(pc_x));
res_lat = VOLUME_WIDTH / (max(pc_y) - min(pc_y));
res_axi = VOLUME_HEIGHT / (max(pc_z) - min(pc_z));
...
figure();
vol = volshow(volume, 'BackgroundColor', [1 1 1], 'Lighting', false, ...
    'Colormap', colormap, 'Alphamap', alphamap, 'Renderer', 'isosurface');

% imagesc(flipud(volume_raw(:,:,16)))   % check one slice
